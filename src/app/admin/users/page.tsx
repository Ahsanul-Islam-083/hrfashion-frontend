  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted text-sm">{data?.total ?? 0} registered users</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 px-4 py-2.5 bg-card border border-card-border rounded-sm text-sm focus:outline-none focus:border-foreground"
        />
        <select value={searchParams.get("role") || ""} onChange={e => updateParam("role", e.target.value || null)} className="px-4 py-2.5 bg-card border border-card-border rounded-sm text-sm focus:outline-none">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-card border-b border-card-border">
              <tr>{["User", "Email", "Role", "Joined", "Actions"].map(h => <th key={h} className="px-5 py-3.5 font-medium uppercase tracking-widest text-xs text-muted">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-card-border">
              {isLoading ? (
                <tr><td colSpan={5} className="px-5 py-10 text-center text-muted">Loading...</td></tr>
              ) : data?.users.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-16 text-center text-muted">No users found.</td></tr>
              ) : data?.users.map(user => {
                  ...
                  <tr key={user.id} className="hover:bg-foreground/5 transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-card flex items-center justify-center text-xs font-medium uppercase overflow-hidden">
                          {user.image ? <img src={user.image} alt={user.name} className="w-full h-full object-cover" /> : user.name?.charAt(0)}
                        </div>
                        <span className="font-medium">{user.name}</span>
                        {isSelf && <span className="text-[10px] uppercase tracking-widest text-muted">(You)</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-card-border rounded-sm hover:bg-foreground/5 disabled:opacity-50 transition-colors"
      ...
      {confirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-background w-full max-w-sm rounded-sm border border-card-border p-6 space-y-4">
            ...
            <p className="text-sm text-muted">This is permanent and will remove all their data.</p>
...
export default function AdminUsersPage() {
  return <Suspense fallback={<div className="text-muted">Loading...</div>}><UsersContent /></Suspense>;
}
