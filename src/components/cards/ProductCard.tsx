...
export function ProductCard({
  ...
  return (
    <div className="group flex flex-col bg-card rounded-sm border border-card-border overflow-hidden hover:border-accent/40 hover:shadow-lg transition-all duration-300">
      {/* Image container */}
      <Link href={`/collections/${_id}`} className="block relative aspect-[3/4] bg-background overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-ink/80 via-ink/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex items-end justify-center pb-6">
          <span className="px-5 py-2.5 bg-pure-white text-ink text-xs font-semibold uppercase tracking-widest rounded-sm transform translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
            View Details
          </span>
        </div>
      </Link>
